...
export const GET = async (req: Request) => {
    try {
        ...
        const rando = Math.floor(Math.random() * 100).toFixed()
        const payload: ActionGetResponse = {
            title: "FABS.fun",
            icon: new URL(`/api/og?number=${rando}`, requestUrl.origin).toString(),
            description: "If you ❤️ something, let it 🔥",
            label: "Burn", // this value will be ignored since `links.actions` exists
            links: {
                actions: [
                    {
                        label: `Burn ${rando} FAB`, // button text
                        href: `${baseHref}${rando}`,
                    },
                    {
                        ...
                        parameters: [
                            {
                                ...
                                label: "Or nominate your own 🔥 amount?", // placeholder of the text input
                                required: true,
                            },
                        ],
                    },
